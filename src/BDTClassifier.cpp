...
BDTClassifier::BDTClassifier (string weightpath):btagMcut(0.8){
    ...
    readerMap["5j4t"]->AddVariable("best_higgs_mass", &variableMap["best_higgs_mass"]);
    ...
    readerMap["6j4t"]->AddVariable("dr_between_lep_and_closest_jet", &variableMap["dr_between_lep_and_closest_jet"]);
...
void BDTClassifier::ResetVariableMap(){
    for(auto it = variableMap.begin(); it != variableMap.end(); it++){
	it->second=-999.;
    }
}

void BDTClassifier::SetCategory(const std::vector<TLorentzVector>& selectedLeptonP4, 
...
double BDTClassifier::GetBDTOutput(const std::vector<TLorentzVector>& selectedLeptonP4, 
				   const std::vector<TLorentzVector>& selectedJetP4, 
				   const std::vector<double>& selectedJetCSV, 
...
std::string BDTClassifier::GetCategoryOfLastEvaluation() const{
    return category;
}

std::map<std::string,float> BDTClassifier::GetVariablesOfLastEvaluation() const{
    return variableMap;
}
